(function( $, undefined ) {
	...
	/*
	 * JMSlideshowMasVisto object
	 */
	$.JMSlideshowMasVisto 				= function( options, element ) {
	...
	$.JMSlideshowMasVisto.prototype 	= {
		_init 				: function( options ) {
			
			this.options 		= $.extend( true, {}, $.JMSlideshowMasVisto.defaults, options );
			
			// each one of the slides
			this.$slides		= $('#jms-slideshowMasVisto').children('div');
		...
		_loadEvents			: function() {
			...
			if( this.$arrowPrev && this.$arrowNext ) {
				...
				this.$arrowPrev.on( 'click.jmslideshowmasvisto', function( event ) {
				...
				this.$arrowNext.on( 'click.jmslideshowmasvisto', function( event ) {
			...
			if( this.$dots ) {
				...
				this.$dots.children().on( 'click.jmslideshowmasvisto', function( event ) {
			...
			this.$jmsWrapper.on( 'touchend.jmslideshowmasvisto', function() {
	...
	$.fn.jmslideshowmasvisto		= function( options ) {
		...
		if ( typeof options === 'string' ) {
			...
			this.each(function() {
				...
				var instance = $.data( this, 'jmslideshowmasvisto' );
				
				if ( !instance ) {
					logError( "cannot call methods on jmslideshowmasvisto prior to initialization; " +
					"attempted to call method '" + options + "'" );
					return;
				}
				
				if ( !$.isFunction( instance[options] ) || options.charAt(0) === "_" ) {
					logError( "no such method '" + options + "' for jmslideshowmasvisto instance" );
					return;
				}
			...
			this.each(function() {
				...
				var instance = $.data( this, 'jmslideshowmasvisto' );
				if ( !instance ) {
					$.data( this, 'jmslideshowmasvisto', new $.JMSlideshowMasVisto( options, this ) );
				}
			});
